...
impl CarRepository {
    ...
    pub fn save(drom_bull: &mut Car) -> i64 {
        let conn = sqlite_connection_factory::get();

        let mut stmt = conn.prepare("INSERT INTO cars (external_id, firm, model, year, complectation, motor_volume, motor_power, fuel, kpp, privod, probeg, price, location, date, created_at, system, exclusive, new, link) 
        VALUES (:external_id, :firm, :model, :year, :complectation, :motor_volume, :motor_power, :fuel, :kpp, :privod, :probeg, :price, :location, :date, :created_at, :system, :exclusive, :new, :link)").unwrap();
        ...
        match stmt.execute(named_params! {
            ...
            Err(err) => {
                error!("creation error: {}", err); 
                0
            },
        }
    }

    pub fn get_identical(external_id: &str, price: u32, system: &str) -> Car {
        let conn = sqlite_connection_factory::get();
    ...
    pub fn get_by_id(id: u64) -> Car {
        let conn = sqlite_connection_factory::get();

        let mut stmt = conn.prepare("SELECT * FROM cars WHERE id = :id limit 1").unwrap();
